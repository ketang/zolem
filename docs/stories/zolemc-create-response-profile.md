...
## Expected Behavior
On success, zolemc prints 'profile <name> created' and exits 0. With -json it prints the profile JSON. If -backend is omitted it defaults to lorem. If the backend value is not one of lorem, faker, fixture, ollama, wasm, or error, the admin API returns 400 and zolemc exits with an error. If -wasm-module-file is set without -backend wasm (or with -backend set to something else), zolemc exits with an error before making the request. If -wasm-timeout-ms is set without wasm backend, zolemc exits with an error. If -response-model-policy is force_literal but -response-model is not set, the admin API returns 400. If a profile with the same name already exists it is replaced (upsert semantics). If the admin server is unreachable, zolemc exits with a network error.
...
- zolemc exits error before API call if -wasm-module-file and -backend is not wasm
- zolemc exits error before API call if -wasm-timeout-ms set without wasm backend
