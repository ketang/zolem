...
func TestLocalRuntimeErrorBackend_E2E(t *testing.T) {
	...
	t.Run("openai-rate-limit", func(t *testing.T) {
		...
		resp, body := doRequest(t, listenerBaseURL, http.MethodPost, "", "/v1/chat/completions", `{"model":"gpt-4o","messages":[{"role":"user","content":"hello"}]}`, map[string]string{
	...
	t.Run("anthropic-rate-limit", func(t *testing.T) {
		...
		resp, body := doRequest(t, listenerBaseURL, http.MethodPost, "", "/v1/messages", `{"model":"claude-3-5-sonnet-20241022","max_tokens":32,"messages":[{"role":"user","content":"hello"}]}`, map[string]string{
			"Content-Type": "application/json",
			"x-api-key":    "sk-test",
		})
		defer resp.Body.Close()
	...
	t.Run("gemini-rate-limit", func(t *testing.T) {
		...
		resp, body := doRequest(t, listenerBaseURL, http.MethodPost, "", "/v1/models/gemini-2.0-flash:generateContent", `{"contents":[{"role":"user","parts":[{"text":"hello"}]}]}`, map[string]string{
			"Content-Type":   "application/json",
...
func startLocalAdminService(t *testing.T, repoRoot string) *localAdminService {
	...
	for time.Now().Before(deadline) {
		...
		resp, _, err := doRequestRaw(client, svc.baseURL, http.MethodGet, "", "/_zolem/health", "", nil)
		if err == nil {
			resp.Body.Close()
...
func createRuntimeListener(t *testing.T, admin *localAdminService, provider string, profile map[string]any) string {
	...
	profileResp, body := doRequest(t, admin.baseURL, http.MethodPut, "", "/_zolem/profiles/"+profileName, string(profileBody), map[string]string{
		"Content-Type": "application/json",
	})
	...
	t.Cleanup(func() {
		resp, _, err := doRequestRaw(&http.Client{Timeout: 5 * time.Second}, admin.baseURL, http.MethodDelete, "", "/_zolem/profiles/"+profileName, "", nil)
		if err == nil && resp != nil {
			resp.Body.Close()
		}
	})
	...
	listenerResp, listenerBody := doRequest(t, admin.baseURL, http.MethodPut, "", "/_zolem/listeners/"+listenerName, listenerPayload, map[string]string{
		"Content-Type": "application/json",
	})
	...
	t.Cleanup(func() {
		resp, _, err := doRequestRaw(&http.Client{Timeout: 5 * time.Second}, admin.baseURL, http.MethodDelete, "", "/_zolem/listeners/"+listenerName, "", nil)
		if err == nil && resp != nil {
			resp.Body.Close()
		}
	})
	...
	stateResp, stateBody := doRequest(t, payload.BaseURL, http.MethodGet, "", "/_zolem/state", "", nil)
